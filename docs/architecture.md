...
```mermaid
flowchart TD
    A[enqueue process_lecture_job] --> B[Storage\nensure_local_file(slides_url, audio_url)]
    B --> C[GeminiClient.upload_pdf]
    B -->|optional audio| D[ElevenLabsClient\nSTT -> Transcript.raw_text]
    ...
    G --> K[render_csv(cards) -> storage\nlecture.csv_url]
    J --> L[PdfNoteService.render_notes_pdf\nembed notes into PDF -> storage]
...
```mermaid
flowchart LR
    PDF[(Slide PDF\napplication/pdf)] --> P[Gemini parts]
    TXT[[TRANSCRIPT_RAW]] --> P
    META[meta block\n(title/subject/professor)] --> P
