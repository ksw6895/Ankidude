...
FastAPI + Celery 백엔드와 Next.js 프론트엔드를 이용해 **슬라이드(PDF)만으로도** Anki Basic CSV를 생성하고, 녹음 파일을 함께 업로드하면 ElevenLabs Scribe v1 STT를 거쳐 더 정제된 결과를 제공합니다. `guideline.md`의 지침(모델/포맷/상태 플로우)을 따릅니다.
...
2. Celery 워커가 상태를 `RUNNING_STT`(오디오 있을 때만) → `RUNNING_LLM` → `GENERATING_CSV`로 갱신  
3. 오디오가 있으면 ElevenLabs `POST /v1/speech-to-text`(`model_id=scribe_v1`) 호출 → transcript 저장, 없으면 빈 transcript로 진행  
